...
/**
 * Decode an i128 SCVal event value into a plain JS string.
 * The runtime value is either a raw object {i128: {hi, lo}} or
 * {i256: ...} depending on the SDK version.
 */
function decodeI128(val: unknown): string {
  ...
  if (v.i128 && typeof v.i128 === 'object') {
    ...
    const hi = parts.hi ?? 0;
    const lo = parts.lo ?? 0;
    // hi is signed 64-bit; combine into a BigInt (avoid literal syntax for TS < ES2020)
    const big = BigInt(hi) * BigInt('18446744073709551616') + BigInt(lo >>> 0);
    return big.toString();
  }
...
/**
 * Decode an address ScVal into its G... or C... string.
 * Runtime shape: {address: {account: {publicKey: string}}}
 * or {address: {contract: {contractId: string}}}.
 */
...
/**
 * Decode a symbol ScVal to its string name.
 * Runtime shape: {sym: "...string..."}.
 */
function decodeSymbol(val: unknown): string {
  if (!val || typeof val !== 'object') return '';
  const v = val as Record<string, unknown>;
...
export default function EventHistory() {
  ...
  // Keep a cursor (paging token) for "next page".
  ...
  const normalizeEvents = useCallback(
    (events: any[]): EventRow[] =>
      events.map((ev: any) => {
        const topic: any[] = ev.topic ?? [];
        // topic[0] is the event name symbol (ScVal with {sym: "..."})
        const name = decodeSymbol(topic[0]);
        ...
        return {
          ...
          timestamp: ev.ledgerClosedAt
            ? new Date(ev.ledgerClosedAt).toISOString()
  ...
  const fetchEvents = useCallback(
    async (cursor: string | null, append: boolean) => {
      ...
      try {
        ...
        const res: any = await server.getEvents(opts);
        ...
        if (events.length > 0) {
          cursorRef.current = events[events.length - 1].pagingToken ?? null;
        }
      } catch (err) {
